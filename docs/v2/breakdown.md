...
- [~] **OAuth 2.1 PAR (Pushed Authorization Requests)** — pure planner `src/core/oauthPushedAuthRequest.ts` (10 тестов). Per-provider config `src/core/parProviderRegistry.ts` (4 провайдера, все `parEndpointUrl: null`); 4 теста. **Sign-in orchestrator** `src/core/parSignInOrchestrator.ts:runParThenAuthorize` discriminated `{ kind: "par_used" | "fallback_to_pkce" | "error" }` — orchestrator iterates: getConfig → buildBody → POST → parseResponse → buildAuthorizeUrl. Caller остаётся agnostic; 2 теста на fallback path. Real PAR endpoint появится сам когда провайдер объявит, fallback_to_pkce — текущая поведение.
