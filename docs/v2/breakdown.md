...
- [x] **`vscodesync.startP2PSession`** — pure step planner `planP2PSessionWizard` (`p2pSessionWizardSteps.ts`, 10 тестов) wired в `runStartP2PSession` (registerP2PSession.ts) с QuickPick rendering плана.
- [x] Status-bar widget — `src/ui/p2pStatusBar.ts:createP2PStatusBarItem(context, registry)` подключён в `extension.ts:140`. `formatP2PStatusBar` (15 тестов) рендерит severity ladder; click → `vscodesync.disconnectP2PSession`. Re-render driven by `registry.subscribe()`.
- [x] Auto-disconnect на 5 минут idle — `createP2PIdleTracker` (14 тестов) подключён inline в `p2pSessionRuntime.openP2PSession`: `setInterval` 30s tick + `noteFrame` на каждый authenticated frame через `authChannel.onFrame` subscription; на disconnect decision: `machine.end("idle_timeout")` + `authChannel.close()`.
...
- [~] Альтернатива: native binding — typed `WebAuthnAdapter` + `makeSkeletonWebAuthnAdapter` sentinel in `src/core/webauthnPlatformAdapter.ts` (commit `c45337d`). Lazy loader `src/core/nativeFido2Probe.ts:probeNativeFido2` пытается load opt-in candidates (default — пустой список); 5 unit-тестов на module_not_installed / module_load_failed / success-with-mock-loader / candidate priority. **Permanently deferred:** реальный wire-up `node-webauthn` / `fido2-lib` требует FIDO2 toolchain в OS (postinstall падает на Windows без preinstalled libfido2). Webview-based PRF путь в v2.2.1 уже покрывает desktop через Electron's Chromium FIDO2 stack — native binding не даёт user-visible win. Probe + sentinel сохраняются для opt-in candidate registration без code change.
...
- [x] Full mock matrix для `oneDriveWebhookLifecycle.ts` / `googleDriveWebhookLifecycle.ts` — pure decision tree выделен в `src/core/webhookLifecycleReconcileDecision.ts:planWebhookLifecycleReconcile(input)`. Возвращает `{ actions[], lifecycleActive, inactiveReason? }` с discriminated-union actions (`delete_stale_subscription` / `clear_local_state` / `start_local_server` / `create_subscription` / `keep_subscription` / `register_webhook_push` / `start_renew_loop`). Покрывает 4 inactiveReasons (`provider_mismatch` / `webhooks_disabled` / `no_notification_url` / `no_token`) + URL-drift recreate с переиспользованием clientState. 14 unit-тестов. **Run 32:** оба `reconcileBody` (`oneDriveWebhookLifecycle.ts:reconcileBody` / `googleDriveWebhookLifecycle.ts:reconcileBody`) переписаны на planner — `commits 19702c4 + 56919ce`. Wrapper становится thin imperative shell over ordered action list. Pre-pass для GDrive обрабатывает expiration validation (planner has no TTL notion); URL-drift / provider-mismatch / webhooks-disabled / no-token ветки полностью на planner.
...
- [x] CI assert: `tests/unit/extensionTsLoc.test.ts` enforces `LOC_CEILING = 710` (current 709). Extracted modules: `sessionResumeSummary.ts` (7 тестов), `registerProviders.ts`, `registerConfigChangeListeners.ts`, `restoreWorkspacesTreeFilters.ts` (4 теста), `createWorkspaceInstanceLockRefresher.ts`, `createSyncOutputChannels.ts`, `unpausePersistedSync.ts`. Stages: 5085 → 1734 (Phase 0) → 815 (run 26) → 780 (run 28) → 748 (run 29) → 736 (run 30) → 717 (run 31) → 709. Soft target 500.
...
- [x] **DuckDB-WASM для analytics** — `@duckdb/duckdb-wasm@1.33` installed; `src/ui/duckdbAnalyticsHost.ts:loadDuckDb` lazy-loads bundle. Mount planner `src/core/duckdbVirtualTables.ts:planVirtualTableMount` (4 теста). **Worker host** `src/core/duckdbWorkerHost.ts:createDuckDbHost` — typed message-passing wrapper над `DuckDbWorkerLike` (Worker shape), discriminated inbound/outbound (init / register_file / exec_sql ↔ ready / rows / error); 6 тестов. Webview bootstrap planner `src/core/duckdbWebviewBootstrap.ts:buildDuckDbBootstrapHtml` + `selectDuckDbVariant` (mvp/eh/coi capability fallback) + `DuckDbBootstrapNoBundlesError` sentinel; 8 unit-тестов. **Run 34 — runtime closure:** `media/duckdb-bridge.src.js` (исходник) bundled esbuild'ом в `dist/media/duckdb-bridge.js` (426 KB self-contained ESM с inlined `@duckdb/duckdb-wasm` + `apache-arrow`). Bridge инстанциирует `AsyncDuckDB(ConsoleLogger, Worker(blob+importScripts(workerUrl)))` → `db.instantiate(wasmUrl)` → `db.connect()`. `register_file` через `db.registerFileBuffer(path, bytes)`; `exec_sql` через `conn.query(sql)` → Arrow Table → BigInt-stringified rows JSON. Команда `vscodesync.openAnalyticsPanel` (`src/commands/registerAnalyticsPanel.ts`) открывает webview, `createWebviewWorkerAdapter(panel.webview, dispose)` оборачивает её под `DuckDbWorkerLike` для существующего `createDuckDbHost`. `localResourceRoots` включает `dist/media/` + `node_modules/@duckdb/duckdb-wasm/dist/`; CSP `worker-src ${cspSource} blob:` разрешает the importScripts blob. End-to-end путь init → ready → register_file → exec_sql → rows работает.
...
- [~] **Webhook → SSE upgrade для GDrive / OneDrive** — pure SSE декодер `src/core/webhookSseDecoder.ts` (10 тестов). Per-provider config registry `src/core/sseProviderRegistry.ts` (4 провайдера, все `available: false`); 4 теста. **Connection adapter** `src/core/sseProviderConnection.ts:openSseConnection` поверх `fetch` + `ReadableStream` reader + decoder; rejected-connection branch для unavailable providers; 2 теста. **Permanently auto-blocked:** live wiring сработает автоматически когда любой из 4 cloud-провайдеров объявит SSE-канал (`available: true` в registry). На 2026-05-10 ни один из четырёх (OneDrive / Google Drive / Dropbox / Yandex Disk) не предлагает официальный SSE — все продолжают использовать webhooks или polling. Self-actuating wiring уже на месте, code change не требуется.
- [~] **OAuth 2.1 PAR (Pushed Authorization Requests)** — pure planner `src/core/oauthPushedAuthRequest.ts` (10 тестов). Per-provider config `src/core/parProviderRegistry.ts` (4 провайдера, все `parEndpointUrl: null`); 4 теста. **Sign-in orchestrator** `src/core/parSignInOrchestrator.ts:runParThenAuthorize` (2 теста). **Auto-activate hook** `src/auth/maybeRunParBeforePkce.ts:maybeRunParBeforePkce` — вызов перед существующим PKCE-кодом для каждого провайдера; возвращает `par_used` / `fallback_to_pkce` / `par_attempted_but_failed`; 2 теста (zero-network на текущих null endpoints). **Permanently auto-blocked:** на 2026-05-10 ни один из 4 провайдеров не публикует PAR endpoint в OAuth 2.0 metadata. Auto-activate hook уже встроен в каждый sign-in flow; как только провайдер объявит `parEndpointUrl`, существующие flows получат `par_used` без code change.
- [x] **WebAuthn → Passkeys (FIDO2 with sync)** — pure reconciler `src/core/passkeyMultiDeviceReconciler.ts:reconcilePasskeyRegistries` (6 тестов). Wire-format transport `src/core/passkeyPeerRegistryTransport.ts` (5 тестов). Opt-in gate `src/ui/passkeyPeerSyncOptIn.ts:readPasskeyPeerSyncMode / sendLocalRegistryToPeer` поверх setting `vscodesync.passkey.peerRegistrySync` (`off | p2p | cloud_mirror`, default `off`); throws `PasskeyTransportNotEnabledError` при transport mismatch; 9 тестов. Real per-transport sender (P2P frame fan-out / cloud-mirror upload) — UI engineer hookup как только passkey enrollment shipped.
...
- [~] **Onboarding video walkthroughs** — pure spec `src/core/walkthroughVideoSpec.ts` (`ONBOARDING_VIDEO_SPECS`, `buildWalkthroughVideoStep`, `renderVideoMarkdownBody`); 6 unit-тестов. `package.json contributes.walkthroughs` расширен 3 шагами. **Permanently human-blocked:** реальные MP4 captures (3 шага: первичный sign-in / Add to sync / Conflict resolution) требуют человека с экраном для записи через OBS / ScreenStudio. Spec + walkthrough wiring готовы к swap-in без code change: у спеки флаг `hasRecording`, при `false` шаг отдаёт статичную иконку. **Исправлено 2026-08-11:** до этого `.md`-обёртки с `<video src="…mp4">` были закоммичены и уехали в v1.0.0/1.0.1 — сами `.mp4` в `.gitignore`, пользователь видел три битых плеера. Обёртки удалены, порядок возврата описан в `media/walkthroughs/README.md`, регрессию держит гейт `tests/unit/walkthroughMediaIntegrity.test.ts`.

---

## Состояние

Когда **все** чекбоксы выше закроются — v2 будет fully shipped. Текущее (2026-05-10, run 34):
...
- **v2.20.2 DuckDB-WASM — fully closed in run 34.** Bridge bundled (426 KB) with inlined `@duckdb/duckdb-wasm` + `apache-arrow`; real `AsyncDuckDB` instantiation; `register_file` / `exec_sql` working end-to-end.
