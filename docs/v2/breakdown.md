...
- [~] **Onboarding video walkthroughs** — pure spec `src/core/walkthroughVideoSpec.ts` (`ONBOARDING_VIDEO_SPECS`, `buildWalkthroughVideoStep`, `renderVideoMarkdownBody`); 6 unit-тестов. `package.json contributes.walkthroughs` расширен 3 шагами с `media.markdown` ссылками; `media/walkthroughs/*.md` checked-in, `*.mp4` в `.gitignore` до реальной записи. **Permanently human-blocked:** реальные MP4 captures (3 шага: первичный sign-in / Add to sync / Conflict resolution) требуют человека с экраном для записи через OBS / ScreenStudio. Spec + markdown stubs + walkthrough wiring готовы к swap-in без code change.
