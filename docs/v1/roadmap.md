...
- [~] **Этап 3. Политика «ничего без спроса»** — `mutationPolicy` как единственный чекпоинт,
  ...
  - [x] **3.1 (F2) — единый чекпоинт мутации** — ✅ (2026-07-31, ветка `next`)
    ...
    - [ ] **Вне политики мутаций, но найдено тем же проходом:** `p2pFileTransferReceiver`
          делает `path.join(root, manifest.relPath)` без проверки на traversal, тогда как
          соседний приёмный путь использует `safePosixRelative` — запись за пределы
          воркспейса по данным из сети. Плюс `commandCenter`/`settingsPanel` выполняют
