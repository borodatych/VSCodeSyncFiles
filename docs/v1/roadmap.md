...
- [ ] **Follow-up (согласовать)**: нативный `moveFile` в реальных провайдерах;
      фоновый GC сирот (блобы/_meta/история под мёртвыми ключами, только через
      корзину провайдера); история на `linkId` + Timeline; кнопка «Починить
      дубликаты» из health check.
