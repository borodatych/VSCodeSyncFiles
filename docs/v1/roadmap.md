...
- [x] **Этап 4. Провайдеры** — ✅ (2026-08-06, ветка `next`)
  ...
  - [x] **4.5–4.6 — E6, E7, E10, E13** — ✅ (2026-08-06, ветка `next`)
    ...
    - [x] **E13** — `createFolder` у OneDrive был пустой заглушкой (три других
          ...
          412-merge `_meta`. Неподключённые PKCE-обёртки OneDrive/Drive
          зафиксированы в `docs/v2/deferredWiring.md`: это не рефакторинг, а
          смена пути входа, требующая регистрации redirect URI в приложении
          пользователя.
  - [ ] **E11** — требует прогона против живого Google Drive API (отдаёт ли
        Drive v3 заголовок ETag на PATCH/multipart-POST). Статически не решается.
