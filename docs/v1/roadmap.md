...
- [ ] **Этап 5. Рефакторинг ядра** — `syncEngine.ts` → оркестратор < 600 строк
      плюс слои `plan/` и `io/`. **Отложено решением владельца 2026-08-13:**
      берётся отдельным заходом со своим планом, не «между делом» — семикратное
      сокращение задевает ~70 точек ключевания манифеста под живым гейтом
      `manifestKeyUsage`, и частичная перетасовка даёт худшее из двух (движок
      всё ещё большой, но уже переехал). Фактический размер на момент решения —
      4116 строк: потолок держится выносами по ходу фич (`manifestHealthOps`,
      `io/trackedBlobReader`, `io/orphanGcOps`, `io/canonicalRelocation`,
      `linkIdHealthOps`), и это рабочая тактика до отдельного захода.
...
- [x] **Дерево показывает структуру воркспейса** — ✅ (2026-08-13, ветка
      `next`) канон-режим стал дефолтом и запоминается между запусками
      (`WORKSPACES_CANONICAL_MODE_KEY`): группировка по локальному размещению
      прятала результат правки облачных путей (байты не двигаются — двигаются
      ключи), и владелец, подняв папки в корень, видел старое дерево. Плюс:
      плоский список (`groupFilesByFolder: false`) теперь работает в обоих
      пространствах, узел воркспейса помечен «как у меня» вне канон-режима,
      подписи переключателя и тултип названы через засев.
- [x] **Мануал и подача киллер-фичи «разная структура папок»** — ✅ (2026-08-13,
      ветка `next`) `docs/manuals/crossMachinePaths.md` (172 строки, стиль
      мануалов VibeIDE: «Отвечает на вопросы», предусловия отдельной секцией,
      три нумерованных этапа — связать / поправить в облаке / засеять вторую
      машину, таблица «Если не работает», строка «Связанное»); `docs/README.md`
      создан с нуля как дерево документации; в README киллер-фича поднята первым
      буллетом и развёрнута до абзаца (было две сухие строки), добавлен раздел
      «Документация» — ссылки абсолютные, т.к. `.vscodeignore` выкидывает
      `docs/**` из `.vsix`, а README служит страницей маркетплейса; в
      `functional.md` — обратные указатели «Пошагово — мануал». Все названия
      команд в мануале сверены с `package.nls.json` скриптом.
- [x] **Контекстное меню Explorer перестало зависеть от активного редактора** —
      ✅ (2026-08-13, ветка `next`) одно подменю обслуживало и Explorer, и
      редактор, а 11 из 13 его пунктов гейтились по `vscodeSync.activeFileTracked`
      — то есть по файлу в редакторе, а не по тому, по которому щёлкнули:
      открытый отслеживаемый файл прятал «Привязать…» и «Добавить…» у любого
      файла в дереве проводника, а «Отправить/Скачать/Сравнить» показывал у
      неотслеживаемых. Заведено отдельное `vscodesync.explorerSubmenu` (11
      пунктов, условие только по кликнутому ресурсу); редакторные поверхности
      сохранили прежнее подменю, где гейт по активному файлу корректен. Все
      команды принимают `uri` и сами сообщают о состоянии («файл не в
      синхронизации»), поэтому пункты безопасно показывать всегда.
      Конфликтные `keepMine`/`takeTheirs` в Explorer не выносились — там нет
      видимого состояния конфликта.
- [x] **`treeFolderBind` / `treeFolderPull` убраны из палитры** — ✅
      (2026-08-13, ветка `next`) обработчики выходят на `kind !== "fileFolder"`,
      из палитры команды молча ничего не делали; добавлено `when: false`.
- [x] **Мануал «Свой OAuth-ключ провайдера»** — ✅ (2026-08-13, ветка `next`)
      `docs/manuals/ownOAuthApp.md`: закрыто висевшее в README обещание
      «есть инструкция, как подключить свой токен» — инструкции не существовало.
      Таблица «настройка / redirect URI / права» по всем четырём провайдерам
      (порты 8734–8737 и scope сверены с кодом PKCE-флоу), шаги регистрации
      приложения, предупреждение про обязательный повторный вход после смены
      ключа, таблица «Если не работает», разграничение с ручным вводом токена.
- [x] **Подача киллер-фичи вне README** — ✅ (2026-08-13, ветка `next`) четыре
      поверхности, все без съёмки: шаг онбординга «Структура папок может
      отличаться» (markdown-медиа `media/steps/crossMachineStructure.md` —
      схема путей работа/облако/дом, кнопка на `bindLocalFolder`; вставлен
      после «Добавьте файл», где вопрос и возникает); строка в `viewsWelcome`
      пустой панели; контекстная подсказка `files_missing_local` (порог 3
      файла со статусом «нет на диске» — классический симптом разошедшейся
      структуры, ведёт на привязку папки, а не на массовое скачивание) +
      3 теста; метаданные маркетплейса — `categories` дополнены
      `SCM Providers` (было только `Other`), заведены 12 `keywords`
      (sync/settings sync/onedrive/dropbox/…) и `galleryBanner`. Ссылки на
      мануалы дописаны в уже выпущенный GitHub Release v1.4.1.
- [x] **Фильтр «только расхождения» в дереве** — ✅ (2026-08-13, ветка `next`)
      на 100+ файлах в одном воркспейсе разошедшиеся приходилось искать глазами.
      Кнопка-фильтр `$(filter)` в заголовке панели оставляет файлы со статусом
      не-`ok`; фильтрация идёт по строкам ДО построения дерева, поэтому пустые
      папки исчезают сами. Общий предикат `isDivergedSyncStatus` в `types.ts`
      (undefined ≠ расхождение — иначе непроверенные строки затопили бы
      отфильтрованный вид). Состояние персистится
      (`WORKSPACES_ONLY_DIVERGED_KEY`), в подписи панели — «⚠ только
      расхождения», в статус-баре после включения — счётчик, а при нуле честное
      «расхождений нет» (пустое дерево не должно читаться как поломка).
- [x] **«Обновить» в панели «Расхождения» обновляет и дерево** — ✅ (2026-08-13,
      ветка `next`) кнопка гоняла `checkWorkspaceStatus`, который переписывает
      `syncStatus` в конфиге, но обновляла только webview: дерево оставалось со
      снимком до пересчёта и показывало «pending push» там, где панель уже
      честно давала «↓3». Обработчик `refresh` теперь дёргает `deps.refreshUi()`
      — так же, как это давно делал `runBulk`. Заодно пустой центр под фильтром
      называет число строк под «Все»: пустота при непустой шапке читалась как
      поломка, а не как выбранный чип.

- [x] **Пауза воркспейса, пригодная для прыжков по git-веткам** — ✅ (2026-08-13,
      ветка `next`) Suspend перекрывал только push/pull: детектор
      `checkWorkspaceStatus` шёл мимо проверки `syncState`, поэтому checkout
      заливал дерево «pending push» ровно там, где пользователь просил тишины.
      Теперь (1) детектор молчит на приостановленном/замороженном воркспейсе —
      ранний выход до скачивания манифеста; (2) его строки видны в панели, но
      не идут в `summariseDivergences`, то есть в счётчик, статус-бар и тост;
      (3) в дереве такие строки подписаны «⏸ статус заморожен», у воркспейса —
      «статусы заморожены»; (4) пакетные команды
      `vscodesync.suspendWorkspaces` / `resumeWorkspaces` — quick pick с
      галочками, по умолчанию отмечены все; (5) возобновление (и одиночное, и
      пакетное) обязательно пересчитывает статусы; (6) авто-пауза по смене
      git-ветки для воркспейсов БЕЗ привязки `gitBranch` — чистый планировщик
      `gitBranchPausePlanner` + настройка `vscodesync.git.autoSuspendUnbound`
      (по умолчанию выкл.), поля `lastSeenGitBranch` / `autoPausedFromBranch`
      в записи воркспейса; ручная пауза авто-возобновлением не снимается.
      Попутно: `.git/HEAD` читается напрямую, когда расширения Git нет, —
      без этого вся branch-policy на таких установках была недостижима, а
      `parseGitHead` оставался мёртвым кодом с 3.L. 14 новых тестов.

- [x] **Категория VibeSync вместо префикса в каждом заголовке** — ✅ (2026-08-13,
      ветка `next`) `VSCodeSync: ` был вшит в 182 заголовка из 190, а поля
      `category` не было ни у одной из 189 команд: в контекстных меню префикс
      был шумом (по расширению и так кликают), а у восьми новых команд его
      забыли — и они терялись в палитре. Теперь у всех команд
      `"category": "VibeSync"` (короче и пригодно для будущей интеграции в
      VibeIDE — решение владельца), заголовки без префикса; палитра печатает
      «VibeSync: …» сама. Заодно выправлены «Возобновить Workspace...» →
      «Возобновить воркспейс…» и суффиксы `(Tree)` → `(дерево)` в русском
      каталоге. Гейт `tests/unit/commandTitlesGate.test.ts`: у каждой команды
      есть category, ни один заголовок не начинается с имени продукта, каждый
      ключ переведён в обоих каталогах.
- [x] **Аудит команд меню воркспейса: два дефекта** — ✅ (2026-08-13, ветка
      `next`) (1) «Удалить воркспейс из облака…» в варианте «и удалить
      локально» собирал пути как `path.join(root, localPath)` — единственное
      место в командах мимо `pathMapping`/Link Bindings; на машине с привязками
      `unlink` не находил файлы, промахи глотал пустой `catch`, и тост называл
      заниженное число удалённых. Теперь через `trackedAbsolutePathFor`.
      (2) «Разморозить» после снятия Freeze безусловно гнал
      `repairLocalStateFromCloud` + `syncWorkspace` — данные ехали в обе
      стороны с кнопки, которая обещала только снять заморозку; добавлен тот же
      гейт `confirmTreeWorkspaceBulkSyncIfNeeded`, что у «Разархивировать», с
      честным сообщением при отказе. Остальные 10 пунктов меню проверены —
      делают то, что обещают, `when`-условия по состояниям корректны.

- [x] **Мануал «Пауза и заморозка воркспейса»** — ✅ (2026-08-14, ветка `next`)
      владелец спросил, чем Freeze отличается от Suspend, — при том что оба
      пункта в меню давно, а `functional.md` перечислял состояния одной
      строкой. `docs/manuals/pauseAndFreeze.md`: таблица различий (файлы,
      статусы, запись манифеста/`_meta`, снятие, переходы), сценарии «когда
      что», поведение снимка статусов, пакетные команды, обе автоматики по
      git-ветке и их правила (ручную паузу не снимают, на detached HEAD не
      срабатывают), FAQ. Строка в дереве `docs/README.md`, ссылка из
      `functional.md`.

- [x] **Авто-пауза по ветке вынесена из-под облачного шлагбаума** — ✅
      (2026-08-14, ветка `next`) владелец включил
      `vscodesync.git.autoSuspendUnbound` в VibeIDE — и пауза не сработала.
      Причина: `applyUnboundBranchPause` вызывался после
      `tryAuthenticatedProvider()`, потому что применял решение через движок,
      а движок не строится без провайдера. Любой оффлайн, разлогин или форк со
      своим secret storage молча отключали функцию, которой сеть не нужна
      вовсе. Теперь пауза применяется первой, прямой записью в конфиг через
      `WorkspaceConfigManager.mutate` (чистая `applyBranchPausePlanToConfig`),
      пересчёт статусов при возобновлении — best-effort. Плюс диагностика на
      каждый ранний выход прохода и два гейта: порядок вызовов и «ни одного
      молчаливого return».

- [x] **Сетевой отказ называется по имени, а не «fetch failed»** — ✅
      (2026-09-19, ветка `next`) владелец получил тост «Yandex Disk: fetch
      failed» и спросил, что это. Разбор его же логов VibeIDE: 147 отказов за
      двое суток, все ровно по 10.0–10.5 с — дефолтный connect timeout undici,
      то есть TCP-соединение не устанавливалось (наши таймауты 30 с и 120 с, их
      строка `ABORT after 30000ms` в логе встретилась один раз). Причина внешняя
      — фильтрация на рабочем периметре, — но три дефекта наши: (1) настоящая
      причина лежала в `error.cause` и выбрасывалась и в тост, и в лог; теперь
      её разбирает чистый `transportFailureReason` (коды undici/Node →
      человеческая фраза); (2) сетевая ошибка приходила красной ошибкой с голым
      текстом — стала предупреждением с кнопками «Повторить» (перезапуск той же
      команды) и «Подробнее» (новая команда `showDiagnosticsChannel`); (3) фон
      не реагировал на устойчивую недоступность — добавлен `cloudProbeBackoff`
      (15 с → 5 мин, сброс на первом успехе, дебаунс 5 с против тройного бампа
      из одного `withRetry`), встроенный в `backgroundCloudAllowed()`. Переходы
      режима пишутся строкой в канал. 15 новых тестов.
- [x] **Команда «Проверить связь с облаком»** — ✅ (2026-09-19, ветка `next`)
      отвечает на вопрос «это я, сеть или сервис»: замер ответа, различение
      «медленно / токен отклонён / троттлинг / нет связи» и совет под каждый
      случай (`cloudConnectionReport`, чистый модуль). Итог дублируется в канал
      Diagnostics — тост читают один раз, журнал прикладывают к отчёту.
- [x] **Профиль фоновых опросов — один регулятор вместо трёх констант** — ✅
      (2026-09-19, ветка `next`) проверка связи (30 с), присутствие машин
      (60 с) и заявки на одобрение (75 с) держали свои литералы, и уменьшить
      трафик пользователь не мог никак. Настройка
      `vscodesync.background.pollProfile` (`normal` / `relaxed` / `minimal` /
      `off`) множителем разрежает все три сразу, сохраняя их относительный ритм;
      `off` не заводит таймер вовсе, а не растягивает его до суток. Профиль
      применяется без перезапуска окна.
- [x] **Аудит: протухший тест, скрытая зависимость от часов, дубль подписи** —
      ✅ (2026-09-19, ветка `next`) `conflictHeatmapTimeline` читала `Date.now()`
      внутри, а её тест был написан на майские даты 2026 — с августа прогон стал
      падать сам по себе, без единой правки кода. «Сейчас» теперь приходит
      параметром (`nowMs`), тест детерминирован и проверяет само окно в 90 дней.
      Таблица подписей провайдера жила двумя копиями (`statusBar`,
      `healthCheckReport`) и готовилась к третьей — вынесена в
      `core/providerLabel`. Проверено и признано чистым: команд-призраков в
      манифесте нет (все 191 регистрируются), настроек без чтения из кода нет
      (все 98 используются).

- [x] **Две одинаковые воронки в заголовке панели** — ✅ (2026-09-19, ветка
      `next`) «Фильтровать воркспейсы…» и «Дерево: только расхождения» несли
      обе `$(filter)` и стояли в одном ряду — владелец не мог отличить их
      взглядом. Режим расхождений получил `$(diff)` (иконка сверена по набору
      codicon установленной VS Code 1.131, а не по памяти; перед правкой
      владельцу показана страница с реальными глифами из `codicon.ttf`).
      Гейт `tests/unit/viewTitleIconsGate.test.ts` держит три условия: в
      заголовке панели нет двух кнопок с одной иконкой, за фильтром закреплена
      воронка, а в navigation-группе нет пунктов без иконки.

- [ ] ~~Ролики онбординга и скриншоты~~ — **снято 2026-08-13 решением
      владельца**: продукт нишевый и не продаётся, съёмка — расход времени без
      отдачи. Не предлагать и не напоминать; вернуть только по явной просьбе.
      Механика под видео в коде остаётся рабочей (см. `docs/v2/breakdown.md`).
