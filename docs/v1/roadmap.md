...
- [~] **Этап 3. Политика «ничего без спроса»** — `mutationPolicy` как единственный чекпоинт,
  ...
  - [x] **3.3 — таймерные пути вне движка** — ✅ (2026-07-31, ветка `next`)
    - [x] **B15 — traversal в P2P-приёме закрыт**: `safePosixRelative` + запрет абсолютных
          ...
          входящих с подтверждением — остаток на 3.6.
  ...
  - [x] **3.5 — панель «Расхождения»** — ✅ (2026-07-31, ветка `next`)
    ...
    - [ ] Остаток 3.3: **B7/B8/B9** — пути гейтятся `autoSyncMode=full`, который под
          чекпоинтом уже не двигает данные; их переустройство — вместе с удалением
          режима `full` в 3.4. **B16** (GDrive `listFolder` создаёт папки) — этап 4.4.
          **B14** (`conflictRules`) — удаление в 3.4. Снапшоты (ручные и по кнопке)
          по-прежнему идут мимо E2E-шифрования — известная проблема, чинить при
          выделении `io/`-слоя (этап 5).
