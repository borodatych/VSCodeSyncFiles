...
## Статус: `[~]` (большая часть закрыта в ночной волне)

## Производительность / автоматизация

- [x] **Scheduled Snapshots (cron-like)** — `src/ui/scheduledSnapshots.ts`. Setting `vscodesync.snapshotSchedule` поддерживает `daily HH:MM` и `weekly DOW HH:MM`. Polling 5 min. Retention из `snapshotRetentionDays` + `maxSnapshotsPerWorkspace`.
...
- [~] **Stats Dashboard: heatmap (vanilla CSS-grid)** — `src/ui/activityHeatmap.ts` — vscode-free бакетер (7×24 матрица), 7 unit-тестов. Webview рисует CSS-grid heatmap с opacity-индикацией плотности; без ECharts (не понадобилось, экономия ~500 KB).
- [ ] **Stats Dashboard: sankey** «push from → pull to» — отложено; требует D3 / vis-network.
...
- [~] **Onboarding: link import** — `vscode://borodatych.vscodesyncfiles/connect?provider=…&workspaceId=…` парсится в desktop-`activate` через `vscode.window.registerUriHandler`. Команда `vscodesync.shareWorkspaceLink` копирует ссылку в clipboard. Автодетект провайдера (Edge cookie / Chrome cookie) — отложен.
...
- [~] `src/ui/oneDriveWebhookLifecycle.ts`, `googleDriveWebhookLifecycle.ts` — выделен общий expiration-helper `src/ui/webhookExpirationMath.ts` (`isNearOrPastExpiration`, `reconcileSubscription`), 12 unit-тестов на reconcile decisions (create / renew / none для разных combinations URL+expiry). Полный mock сетевых вызовов lifecycle — отложено.
...
- [ ] Стабилизировать 8 flaky тестов на main (`attachCloudWorkspace`, `conflictResolution`, `engineCallbacks`, `platformCrypto decrypt`) — падают независимо от Hardening-волны.
