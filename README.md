...
- **Привязки** — структура не обязана совпадать: файл или целая папка может лежать на каждой машине в своём месте и под своим именем (на работе `promed/`, дома `php/`) и всё равно синхронизироваться
- **Правка путей воркспейса** — переименуйте файл, папку или целый корень прямо в облачном дереве (команда, drag-and-drop или массовый текстовый редактор): дочерние пути перестроятся, привязки машин не порвутся, локальные файлы никто не двигает
