...
export interface ActiveWorkspaceEntry {
  ...
  gitBranch?: string;
  /**
   * Ветка, на которой этот workspace видели в прошлый проход branch-policy.
   * Нужна воркспейсам БЕЗ привязки (`gitBranch`): без неё «ветку сменили»
   * неотличимо от «ветка всегда была такой».
   */
  lastSeenGitBranch?: string;
  /**
   * Ветка, с которой авто-пауза увела workspace. Пауза, поставленная руками,
   * этого поля не имеет — и авто-возобновление её не трогает: снимать чужую
   * паузу расширение не вправе.
   */
