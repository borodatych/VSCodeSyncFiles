...
- **Две одинаковые воронки в заголовке панели «Воркспейсы».** Фильтр
  воркспейсов и режим «только расхождения» несли одну и ту же иконку и стояли
  в одном ряду. Режим расхождений получил свою — `$(diff)`; воронка осталась
  за фильтром. Добавлен тест, не пропускающий повторную иконку в заголовке
  панели.
...
- **Команда «Проверить связь с облаком».** Отвечает на вопрос «это я, сеть или
  сервис»: обращается к вашему облаку, замеряет ответ и различает «отвечает»,
  «отвечает медленно», «отклонил токен», «ограничивает частоту» и «нет связи» —
  с советом под каждый случай. Итог дублируется в журнал диагностики, который
  открывается кнопкой из того же уведомления.
- **Профиль фоновых опросов.** Настройка `vscodesync.background.pollProfile`
  (обычный / щадящий / минимальный / выключено) одним множителем регулирует все
  собственные обращения расширения к облаку — проверку связи, присутствие машин
  и заявки на одобрение. Раньше их частота была зашита тремя константами.
  Профиль применяется без перезапуска окна.
- **Команда «Показать журнал диагностики»** — прямой путь к каналу, на который
  ссылаются уведомления.

### 🐛 Исправления

- **Сетевой сбой называется по имени.** Вместо `fetch failed` — разобранная
  причина: соединение не установилось за 10 секунд, имя хоста не разрешается,
  соединение отклонено, сертификат перехвачен прокси. Настоящая причина лежала
  в `cause` и выбрасывалась и в уведомление, и в журнал.
- **Потеря связи больше не выглядит аварией команды.** Предупреждение с
  кнопками «Повторить» и «Подробнее» вместо красной ошибки с голой строкой:
  при сетевом сбое файлы не тронуты.
- **Фоновые проверки замолкают, пока облако недоступно.** Разрежение с 15
  секунд до 5 минут и мгновенный возврат к обычному ритму после первого
  успешного ответа. Раньше опросники продолжали заводить новое соединение
  каждую минуту, и каждое висело до таймаута.

### ♻️ Внутреннее

- Чистые модули `transportFailureReason`, `cloudProbeBackoff`,
  `cloudConnectionReport`, `backgroundPollProfile` — вся логика тестируется без
  редактора; 15 новых тестов.
- Подпись провайдера («OneDrive», «Yandex Disk», …) жила двумя копиями и
  готовилась к третьей — вынесена в `core/providerLabel`.
- `conflictHeatmapTimeline` больше не читает системные часы внутри: «сейчас»
  приходит параметром. Её тест был написан на фиксированные майские даты и с
  августа падал сам по себе, упираясь в окно в 90 дней.
